...
fatiga_anterior = False

while True:
    try:
        
        if contador_envios >36:
            
            if  (status_glasses == False and lentes_anterior == True) or \
                ...
                (status_fatiga == True and fatiga_anterior == False) or \
                (status_distraccion == True and distraccion_anterior == False):
                #*,M:{status_head[:2]} ,O:{current_status_eyes[:2]}
                envio = f"DSM,C:{int(status_cellphone)},G:{int(status_cigarrete)},L:{int(status_glasses)},D:{int(status_distraccion)},F:{int(status_fatiga)},T:{int(status_cinturon)}\r\n".encode()
                ser.write(envio)
                ser.flush()
    ...
    utils.display(im, f"OJOS: {current_status_eyes}", 7)
    utils.display(im, f"EAR: {round(status_ear, 2)}", 8)
    utils.display(im, f"MIRADA: {status_head}", 9)
    utils.display(im, f"DISTRACCION: {status_distraccion}", 10)
    #*utils.display(im, f"DISTRACCION: {status_somnolencia}", 11)
    utils.display(im, f"FATIGA: {status_fatiga}", 12)
    utils.display(im, f"QR/BAR:{data_validada}",13)
    utils.display(im, f"CINTURON:{status_cinturon}",14)
